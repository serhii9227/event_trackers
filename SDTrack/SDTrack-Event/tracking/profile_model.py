...
def parse_args():
    ...
    parser.add_argument('--config', type=str, default='SDTrack-base-fe108', help='yaml configure file name')
...
def evaluate_vit(model, template, search):
    ...
    print(flops.by_module())  # 输出每个模块的 FLOPs（MACs 约等于 FLOPs / 2）

    macs1, params1 = profile(model, inputs=(template, search),
                             custom_ops=None, verbose=True)
