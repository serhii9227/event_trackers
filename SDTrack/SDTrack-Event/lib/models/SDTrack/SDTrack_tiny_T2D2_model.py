...
class MultiSpike(nn.Module):
    ...
    def forward(self, x): # B C H W
        spike = torch.zeros_like(x[0]).to(x.device)     # [B, C, H, W]
        output = torch.zeros_like(x)            # [T, B, C, H, W]
        mem_old = 0
        T = x.shape[0]
        for i in range(T):
            if i >= 1:
                mem = (mem_old - spike.detach() * self.max_value) * self.decay + x[i]
            else:
                mem = x[i]
            spike = self.spike_function(mem, min_value=self.min_value, max_value=self.max_value) / (self.Norm)

            mem_old = mem.clone()
            output[i] = spike

            count_050 = torch.sum((spike == 1 / self.Norm).to(torch.int)).item()
            count_1 = torch.sum((spike == 2 / self.Norm).to(torch.int)).item()
            self.fr.append((count_050 * 1 + count_1 * 2) / (spike.numel() * self.Norm))
        # print(self.fr)
